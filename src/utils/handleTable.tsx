...
export interface UseTableFeature {
  ...
  handleSearch: (selectedKeys: Key[], confirm: () => void) => void;
...
export const useTableFeatures = (): UseTableFeature => {
  ...
  const handleTableChange: UseTableFeature["handleTableChange"] = (
    pagination,
    filters,
    sorter
  ) => {
  ...
  const handleSearch: UseTableFeature["handleSearch"] = (
    selectedKeys,
    confirm
  ) => {
    confirm();
    const searchString =
      selectedKeys[0] !== undefined ? String(selectedKeys[0]) : "";
  ...
  const generateColumn: UseTableFeature["generateColumn"] = (
    dataIndex,
    title,
    render = undefined,
    sorter
    ...
    sortOrder: Array.isArray(sortedInfo)
      ? sortedInfo[0]?.columnKey === dataIndex
        ? sortedInfo[0]?.order
        : undefined
